...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame])
    {
        ...
        self.pieRadius = MIN(frame.size.width/2+10, frame.size.height/2+10);
...
- (void)setShowPercentage:(BOOL)showPercentage {
    ...
    for(SliceLayer *layer in _pieView.layer.sublayers)
    {
        ...
        CGSize infoSize = CGSizeMake(60, 20);
...
#pragma mark - Pie Reload Data With Animation   创建 SliceLayer
- (void)reloadData {
    if (_dataSource && !_animationTimer)
    {
        ...
        NSDictionary* dicts[sliceCount];
        for (int index = 0; index < sliceCount; index++) {
            values[index] = [_dataSource pieChart:self valueForSliceAtIndex:index];
            dicts[index] = [_dataSource pieChart:self dictForSliceAtIndex:index];
            sum += values[index];
        }
        ...
        for(int index = 0; index < sliceCount; index ++)
        {
            ...
            layer.percentDict = (sum)?dicts[index]:@{@"title":@"黑头", @"person":@"10", @"amount":@"10234"};
            UIColor *color = nil;
            ...
            if (sliceCount > 1) {
                [layer setStrokeColor:[UIColor clearColor].CGColor];  //整体的底色
...
#pragma mark - Animation Delegate + Run Loop Timer   //放在外面  划线
...
- (void)updateTimerFired:(NSTimer *)timer;
{
    ...
    [pieLayers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        
        {
            ...
            CATextLayer *labelLayer2 = (CATextLayer*)[[obj sublayers] objectAtIndex:2];
            CGFloat interpolatedMidAngle = (interpolatedEndAngle + interpolatedStartAngle) / 2;
            [CATransaction setDisableActions:YES];
            
            if (layer.percentage < self.checkLessPercent) {

                [labelLayer setForegroundColor:[UIColor yellowColor].CGColor];
                [labelLayer2 setForegroundColor:RGB(255, 255, 255).CGColor];
                ...
                float labelLayerX2 = _pieCenter.x + ((_labelRadius + 10)* cos(interpolatedMidAngle));
                
                float labelLayerY2 = _pieCenter.y + ((_labelRadius + 10)* sin(interpolatedMidAngle));
                
                [labelLayer2 setPosition:CGPointMake(labelLayerX2 * 1.0, labelLayerY2 * 1.0)];
...
- (void)setTextAngle:(CGFloat)textAngle {
    ...
    [pieLayers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        CALayer *labelLayer2 = [[obj sublayers] objectAtIndex:2];
        
        // 扇形中百分比Label的旋转角度
        labelLayer.transform = CATransform3DMakeRotation(-textAngle, 0, 0, 1);
        
        labelLayer2.transform = CATransform3DMakeRotation(-textAngle, 0, 0, 1);
        
    }];
}

#pragma mark - Pie Layer Creation Method   添加textLayer
- (SliceLayer *)createSliceLayer {
...
#pragma mark - 这里可以修改整扇形title字体
    // 扇形百分比字体
    [textLayer setFontSize:self.labelFont.pointSize - 3];
    [textLayer2 setFontSize:6];
    // 扇形百分比颜色
    [textLayer setForegroundColor:[UIColor clearColor].CGColor];
    [textLayer setAnchorPoint:CGPointMake(0.5, 0.5)];
    [textLayer2 setForegroundColor:[UIColor clearColor].CGColor];
    [textLayer2 setAnchorPoint:CGPointMake(0.5, 0.5)];
    // 扇形百分比字体居中
    [textLayer setAlignmentMode:kCAAlignmentCenter];
    [textLayer2 setAlignmentMode:kCAAlignmentCenter];
    // 扇形百分比字体清晰度
    [textLayer setContentsScale:[UIScreen mainScreen].scale];
    [textLayer2 setContentsScale:[UIScreen mainScreen].scale];
    // 扇形百分比背景色
    [textLayer setBackgroundColor:[UIColor clearColor].CGColor];
    [textLayer2 setBackgroundColor:[UIColor clearColor].CGColor];
    ...
    [pieLayer addSublayer:textLayer2];
    return pieLayer;
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value {
    ...
    CATextLayer *textLayer2 = (CATextLayer*)[[pieLayer sublayers] objectAtIndex:2];
    [textLayer setHidden:!_showLabel];
    if(!_showLabel) return;
    NSString *label;
    
    NSDictionary *dict = pieLayer.percentDict;
    label = [NSString stringWithFormat:@"￥%@",dict[@"amount"]];
    NSString *string = [NSString stringWithFormat:@"%@\n%@人",dict[@"title"],dict[@"person"]];
  ...
  //  label = [NSString stringWithFormat:@"%.1f%%", pieLayer.percentage];
    ...
    CGSize infoSize = CGSizeMake(80, 20);
    
    CGSize size = [label boundingRectWithSize:infoSize options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes context:nil].size;
    
    NSDictionary *attributes2 = @{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:12]};
    CGSize infoSize2 = CGSizeMake(40, 80);
    
    CGSize size2 = [label boundingRectWithSize:infoSize2 options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attributes2 context:nil].size;
    ...
    [textLayer2 setString:string];
    [textLayer2 setBounds:CGRectMake(0, 0, size2.width , size2.height+5)];
