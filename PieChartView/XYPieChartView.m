...
- (id)initWithFrame:(CGRect)frame withPieChartTypeArray:(NSMutableArray *)pieChartTypeArray withPercentArray:(NSMutableArray *)percentArray withColorArray:(NSMutableArray *)colorArray
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.rotatedView.dataArray = pieChartTypeArray;
        ...
        UIImageView *circle = [[UIImageView alloc] initWithFrame:CGRectMake(5, 5, centerImageSize.width/2-10, centerImageSize.width/2-10)];
        circle.image = [UIImage imageNamed:@"center_white3"];
        [self.centerButton addSubview:circle];
                
        /** 中心圆上展示的标题和总金额 **/
        
        // 标题宽
        CGFloat centerTitleWidth = self.centerButton.frame.size.width-20;
        
        // 标题高
        CGFloat centerTitleHeight = 20;
        
        // 设置标题位置
        self.centerTitle = [[UILabel alloc]initWithFrame:CGRectMake((centerImageSize.width/2 - centerTitleWidth)/2, (centerImageSize.height/2 - 25)/2, centerTitleWidth, centerTitleHeight)];
        
        // 标题背景色
      //  self.centerTitle.backgroundColor = [UIColor redColor];
        ...
        // 金额宽度
        CGFloat amountWidth = self.centerButton.frame.size.width-20;
        
        // 金额宽度
        CGFloat amountHeight = AmountHeight;
        
        // 初始化金额Label 设置位置、大小
        self.centerAmount = [[UILabel alloc]initWithFrame:CGRectMake((centerImageSize.width/2 - amountWidth)/2, 73, amountWidth, amountHeight)];
        
        // 金额背景色
        //self.centerAmount.backgroundColor = [UIColor yellowColor];
        
        // 字体居中
        self.centerAmount.textAlignment = NSTextAlignmentCenter;
        
        // 字体大小
        self.centerAmount.font = [UIFont systemFontOfSize:12];
        
        // 字体颜色
        self.centerAmount.textColor = [UIColor yellowColor];
